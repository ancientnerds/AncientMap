...
Built 2026-09-23T10:39:54+00:00 by `scripts/remediation/mechanical/scope.py` from the production export of 2026-09-23 07:41:01.276622+00. Lane `scope-e4`: run stamp `2026-09-23_mechanical-scope-e4`, journal test id `E4/scope-status`, change keys `scope-e4:<site_id>:<column>`, premise `concat_ws(' | ', coalesce(u.period_start::text, 'NULL'), coalesce(u.period_end::text, 'NULL'), u.lat::text, u.lon::text, coalesce(u.site_type, 'NULL'), u.name, md5(coalesce(u.description, '')))`.

**115 site(s), 230 cell(s): 79 retired, 19 pending, 17 in_scope; 0 refused.** T11 found 96 site(s); 101 curated pair(s) within 100 m share a Wikidata item, 3 of them under two of its names. `output/remediation/bcases/DUPLICATES.jsonl` lists 19 loser(s), 19 of them still holding in the export, 3 also found by this lane's own rule: 19 duplicate(s) in all, each loser counted once.
