...
def render(rows: list[Change], *, reversal: bool, rehearsal: bool = False) -> str:
    ...
    for row in rows:
        ...
        values.append(
            ...
            f"{lanes.sql_text(key)}, {lanes.sql_text(row.test_id)}, {lanes.sql_text(row.confidence)}, {_evidence_json(row)})"
        )
    lines = [
...
def statements(rows: list[Change]) -> dict[str, str]:
...
def assert_rendered(out: pathlib.Path, rows: list[Change]) -> None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--dir", default=str(OUT), help="where the plan and statements live")
    parser.add_argument("--host", default=lanes.HOST)
    args = parser.parse_args(argv)
    out = pathlib.Path(args.dir)
    if args.command == "render":
        rows = write_files(out)
