...
from journal_chain import ROLLBACK_SUFFIX, first_break  # noqa: E402 - on sys.path via lanes

TABLE = "unified_sites"
COLUMNS = ("site_type", "period_start", "country")
#: A reversal's stamp ends in `journal_chain.ROLLBACK_SUFFIX`. Such a row is never counted as a write
#: of the lane - but it is part of the chain, because a kept reversal changed the value.
WINDOW = 200
...
def check_chain(
    ...
    problems: list[str] = []
    where = f"{key[2]} {key[1]}"
    # `journal_chain.first_break` is the one continuity rule the mechanical planners judge chains by
    # too; it is asked again from each break, so every break of the chain is reported, not only the
    # first.
    transitions = [(link.old, link.new) for link in chain]
    start = 0
    while (offset := first_break(transitions[start:])) is not None:
        index = start + offset
        previous, link = chain[index - 1], chain[index]
        problems.append(
    ...
    *,
    ...
    for link in lane_links:
        if link.table != TABLE or link.column not in COLUMNS:
            # A phase-3 writer writes these three fields and nothing else; a lane row anywhere else
            # is a write nobody planned, and dropping it from the count would hide it.
            result.deviations.append(
                f"OUTSIDE THE THREE FIELDS {link.pk} {link.table}.{link.column}: journal row "
                f"{link.id} ({link.stamp}) is a lane write outside {TABLE}.{'/'.join(COLUMNS)}"
            )
            continue
