...
56 row changes (run stamp `2026-09-23_source-url-split-wave4`): 20 `unified_sites.source_url` values keep their first URL, 35 `site_external_ids` rows are new and 1 corrected; 2 value(s) are left, each with its reason. The input is `wave4/RESOLUTION.json`: production read 2026-09-23T10:52:11Z (every `unified_sites` row whose `source_url` carries a control character: 20), English Wikipedia resolved 2026-09-23T10:52:14Z through `pipeline.lyra.prospector.wiki.resolve_titles`. The rules are in the module docstring of `output/remediation/tools/qid_repair.py`.
...
| Acanceh (`e3a400b7-f8cd-4c4d-a2b0-b79ac18d2258`) | enwiki_title | (no row) | `Acanceh` |
| Acanceh (`e3a400b7-f8cd-4c4d-a2b0-b79ac18d2258`) | wikidata_qid | (no row) | `Q8186545` |
...
| Atzompa (`5699564f-61e7-481d-99dc-d23edc32207b`) | enwiki_title | (no row) | `Santa María Atzompa` |
| Atzompa (`5699564f-61e7-481d-99dc-d23edc32207b`) | wikidata_qid | (no row) | `Q3846612` |
...
| Cerro De Trincheras (`d4671d52-1421-4853-a186-6f3b4516368d`) | enwiki_title | (no row) | `Trincheras` |
| Cerro De Trincheras (`d4671d52-1421-4853-a186-6f3b4516368d`) | wikidata_qid | (no row) | `Q1434929` |
...
* After the apply the boot refresh (`refresh_site_external_ids(only_missing=True)`) reads none of these sites again (each either has rows or no longer has an English Wikipedia `source_url`), and the `--all` path resolves Petra's single URL to the rows written here.
