...
describe('useTextureLoading', () => {
  it('puts only the start-tier gray on the critical path', async () => {
    ...
    expect(latest.satelliteOnGpu).toBe(false)
  ...
  it('keeps the satellite active across a context loss and brings back the tier it had', async () => {
    ...
    expect(latest.satelliteOnGpu).toBe(true)
    await act(async () => { loseContext() })
    expect(materials()[0].uniforms.uSatellite.value).toBe(null)
    expect(latest.satelliteReady).toBe(true)
    // nothing to sample until the restore brings a texture back (the shader shows the gray meanwhile)
    expect(latest.satelliteOnGpu).toBe(false)
    await act(async () => { restoreContext() })
    await settle()
    expect(latest.satelliteReady).toBe(true)
  ...
  // Context loss usually comes from memory pressure: the restore must not
  // allocate the 16k satellite (683 MiB) for a satellite nobody shows.
  it('a satellite switched off before the loss comes back at the start tier only', async () => {
    const { refs, materials, loseContext, restoreContext } = makeRefs()
    const p = props(refs)
    await render(p)
    await settle()
    ...
    await settle()
    expect(fetched.filter(u => u === SAT_HIGH)).toHaveLength(1)
    await render({ ...p, satelliteRequested: false })
    await act(async () => { loseContext() })
    await act(async () => { restoreContext() })
    await settle()
    expect(fetched.filter(u => u === SAT_LOW)).toHaveLength(2)
    expect(fetched.filter(u => u === SAT_HIGH)).toHaveLength(1)
    expect((materials()[0].uniforms.uSatellite.value.image as { width: number }).width).toBe(4096)
    expect(trackBackgroundFailure).not.toHaveBeenCalled()
  })

  it('switching the satellite off during the restore aborts its maximum-tier upload', async () => {
    const { refs, materials, loseContext, restoreContext } = makeRefs()
    const p = props(refs)
    await render(p)
    await settle()
    ...
    await act(async () => { restoreContext() })
    await settle()
    // the start tier is back, the maximum tier still decodes
    expect(fetched.filter(u => u === SAT_HIGH)).toHaveLength(2)
    expect((materials()[0].uniforms.uSatellite.value.image as { width: number }).width).toBe(4096)
    await render({ ...p, satelliteRequested: false })
    release()
  ...
  it("switches the satellite off when the restore cannot load it again", async () => {
    ...
    expect(vi.mocked(trackBackgroundFailure).mock.calls[0][0]).toBe('satellite')
    expect(materials()[0].uniforms.uSatellite.value).toBe(null)
    // no maximum tier for a satellite that is gone
    expect(fetched.filter(u => u === SAT_HIGH)).toHaveLength(1)
  })
