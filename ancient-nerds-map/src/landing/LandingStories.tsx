...
function Badge({ category }: { category: string | null }) {
  ...
  return <span className={`ll-badge ll-cat-${category}`}>{getNewsCategoryLabel(category)}</span>
}

function Meter({ value }: { value: number | null }) {
  if (value == null) return null
  return (
    <span className="ll-sig" role="meter" aria-valuenow={value} aria-valuemin={0} aria-valuemax={10} aria-label="significance">
      SIG <b>{value}</b>
...
function StoryLead({ story }: { story: StoryTeaser }) {
  ...
  return (
    <a className="ll-lead" href={story.path}>
      {/* Plain <img>, never LazyImage: that one starts hidden and unhides in
          React's onLoad, which never fires for an image the browser already
          finished before hydration — and never at all without JS. The empty
          span keeps the aspect-ratio box for a story without a screenshot. */}
      <span className="ll-img ll-img-16x9">
        {story.screenshot_url && (
          <img src={story.screenshot_url} alt="" width={1280} height={720} loading="lazy" decoding="async" />
        )}
      </span>
...
function StoryRow({ story }: { story: StoryTeaser }) {
  return (
    <a className="ll-row ll-row-thumb" href={story.path}>
      <span className="ll-img ll-img-16x9">
        {story.screenshot_url && (
          <img src={story.screenshot_url} alt="" width={192} height={108} loading="lazy" decoding="async" />
        )}
      </span>
      <span>
