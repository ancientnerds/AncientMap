...
WD2_CASES: list[Case] = [
    ...
    Case(
        "wd2 scope: round N asks only the uncounted",
        SCOPE_REVIEW,
        '            if r["decision"] == NOT_A_SITE and not r["counted"]',
        '            if r["decision"] == NOT_A_SITE',
    ...
    guard(
        "wd2 scope: a site decided in two rounds",
        SCOPE_REVIEW,
        '                if row["site_id"] in out_rows:',
        "test_a_site_counted_in_two_rounds_is_refused",
        SCOPE_REVIEW_TESTS,
    ),
    ...
    Case(
        "wd2 img: a repaired thumbnail gets the file URL",
        SERVED / "plan.py",
