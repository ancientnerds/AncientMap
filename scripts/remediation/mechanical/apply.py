...
def render_transaction(
    ...
    *,
    ...
    add(f"-- {len(records)} row(s) over {len(sites)} site(s); scope source_id = {_literal(source)};")
    add(f"-- run stamp {_literal(run_stamp)}; journal test id {_literal(TEST_ID)}.")
    add("-- Every UPDATE is conditioned on the old value the plan names; every change is journalled")
    ...
    add(") ON COMMIT DROP;")
    add("")
    add("INSERT INTO _country_plan (site_id, old_value, new_value, change_key, reason, evidence) VALUES")
    rows = []
    ...
    add("    IF bad > 0 THEN")
    add("        RAISE EXCEPTION 'country repair: % planned row(s) no longer hold the planned old "
        "value', bad;")
    add("    END IF;")
    add("")
    ...
    add("    IF bad > 0 THEN")
    add("        RAISE EXCEPTION 'country repair: % planned row(s) do not hold the new value', bad;")
    add("    END IF;")
    add("")
    ...
    add("    IF bad > 0 THEN")
    add("        RAISE EXCEPTION 'country repair: % planned row(s) have no matching journal row', "
        "bad;")
    add("    END IF;")
    add("")
    ...
    add("    IF bad > 0 THEN")
    add("        RAISE EXCEPTION 'country repair: this run stamp journalled % row(s) outside "
        "unified_sites.country', bad;")
    add("    END IF;")
    add("")
    add("    RAISE NOTICE 'country repair: % row(s) changed and journalled over % curated site(s)',")
    add("        moved, expected;")
    add("END $$;")
    add("")
    add("COMMIT;")
    add("")
    add(POST_COMMIT_READS.format(run_stamp=_literal(run_stamp), test_id=_literal(TEST_ID),
...
def rehearse(sql: str, *, run_stamp: str = RUN_STAMP, source: str = CURATED_SOURCE) -> str:
    ...
    return head + "\nROLLBACK;\n" + REHEARSAL_READS.format(
        run_stamp=_literal(run_stamp), source=_literal(source)
    )
...
def verify_interests(records: Sequence[ChangeRecord]) -> str:
    ...
    lines += [f"{country.ljust(width)}  {slug.ljust(23)} {count:>4}" for country, slug, count in rows]
    return "\n".join(lines)
...
def cmd_probe_guards(records: Sequence[ChangeRecord], out: Path) -> int:
    ...
    probes.append(("guard3-foreign-old-value",
                   "guard 3 - a planned old value the row does not hold", corrupted_old))
    ...
    probes.append(("guard1-other-source",
                   "guard 1 - a row outside source_id = 'ancient_nerds'", other_source))
    ...
    for suffix, name, mutated in probes:
        ...
        for line in (proc.stdout + proc.stderr).splitlines():
            if "ERROR" in line or "country repair:" in line:
                print("   " + line.strip())
        left = read_rows(
            f"SELECT count(*) FROM remediation_change_log WHERE run_stamp = '{stamp}'"
        )
