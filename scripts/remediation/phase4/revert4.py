...
def _set(alias: str, pattern: str) -> str:
    """The reverted set: the matched write rows, never their reversals."""
    return (
...
def render_revert(stamp_like: str, *, rehearse: bool = False) -> str:
    ...
    out = [
        ...
        f"-- reverts every journalled write whose run stamp is LIKE {pattern}, newest first,",
        "-- read from remediation_change_log itself; "
        ...
        "    r        RECORD;",
        "BEGIN",
        f"    SELECT count(*) INTO expected FROM remediation_change_log l WHERE {in_set};",
        "    IF expected = 0 THEN",
        ...
        f"              WHERE {in_set} ORDER BY l.id DESC LOOP",
        ...
        *_back_at_old(),
        ...
        "",
        "-- after the transaction: the write rows matched, and their kept reversals",
