...
TERMINAL = ".!?"
#: The one spoken edit of a card (card_texts): `c.`/`ca.` before a number is read "circa".
_SPOKEN_CIRCA = re.compile(r"\b(?:c|ca)\. ?(?=\d)")
...
def spoken(card_sentence: str) -> str:
    """The card's one non-source edit: `c.`/`ca.` in front of a number becomes `circa`."""
