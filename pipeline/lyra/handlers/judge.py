...
class JudgeHandler(BaseHandler):
    ...
    async def _on_paper_ready(self, event: PaperReady):
        ...
        self.emit_sse({"type": "pipeline", "stage": "quality_judge", "status": "start", "meta": {"subtask_total": 1, "attempt": self._judge_attempts}})
        self.emit_sse({"type": "status", "content": f"Running quality judge (attempt {self._judge_attempts})..."})
        ...
        for sid, num in sorted(self.state.registry.reference_numbers.items(), key=lambda kv: kv[1]):
            ...
            if source:
                source_snippets.append({"ref_num": num, "title": source.title, "snippet": source.snippet})
        ...
        self.state.log("judge", f"Score {score}/100 ({badge}), passed={passed}, attempt={self._judge_attempts}")
        ...
        self.emit_sse({
            "type": "pipeline", "stage": "quality_judge", "status": "done",
            "meta": {"score": score, "badge": badge, "passed": passed, "attempt": self._judge_attempts},
        })
        ...
        if passed or score >= self.state.config.quality_pass_threshold:
            ...
            self.state.log("judge", f"Max attempts ({self._judge_attempts}) reached, shipping best (score={self._best_score})")
            self.state.phase = ResearchPhase.DONE
