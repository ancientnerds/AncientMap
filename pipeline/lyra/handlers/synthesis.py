...
class SynthesisHandler(BaseHandler):
    ...
    async def _on_all_saturated(self, event: AllAnglesSaturated):
        ...
        self.emit_sse({"type": "pipeline", "stage": "synthesis", "status": "start", "meta": {"subtask_total": 2}})

        # Step 1: Standard synthesis across all findings
        self.emit_sse({"type": "status", "content": "Synthesizing findings across all angles...", "subtask_done": 0, "subtask_total": 2})

        # Build input: all specialist findings grouped by angle
        angle_summaries = []
        for angle in self.state.angles:
            findings_text = json.dumps(angle.findings, indent=2) if angle.findings else "No findings"
        ...
        async with self.semaphore:
            raw = await asyncio.to_thread(
                minimax_chat_anthropic, synthesis_prompt, user_msg,
                self.state.config.max_tokens_synthesis, _get_settings(),
            )
        self.state.llm_call_count += 1
        self.state.synthesis = self._parse_json(raw)

        # Step 2: Cross-angle connection detection
        self.emit_sse({"type": "status", "content": "Detecting cross-angle connections...", "subtask_done": 1, "subtask_total": 2})
        ...
        if cross_prompt_path.exists():
            ...
            async with self.semaphore:
                raw_cross = await asyncio.to_thread(
                    minimax_chat_anthropic, cross_prompt, user_msg,
                    self.state.config.max_tokens_synthesis, _get_settings(),
                )
            self.state.llm_call_count += 1
            ...
            self.state.log("synthesis", f"Cross-angle: {convergent} convergent, {contradictions} contradictions, {connections} connections")
        else:
        ...
        self.emit_sse({
            "type": "pipeline", "stage": "synthesis", "status": "done",
            "meta": {"consensus": consensus, "contested": contested, "unique": unique, "cross_angle_connections": len(self.state.cross_angle_connections)},
        })
        self.state.log("synthesis", f"Synthesis complete: {consensus} consensus, {contested} contested, {unique} unique")
