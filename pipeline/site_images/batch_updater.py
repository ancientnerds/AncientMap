...
def get_comprehensive_stats(session: Session) -> dict:
    ...
    improvement_potential = session.execute(
        text("""
        SELECT
            source_id,
            COUNT(*) as total,
    ...
    return {
        ...
        "by_type": [
            {"type": r[0], "total": r[1], "with_image": r[2], "coverage_pct": r[3]} for r in by_type
        ],
        "by_country": [
...
def print_stats(stats: dict):
    ...
    for src in stats["by_source"]:
        print(
            f"{src['source']:<30} {src['total']:>10,} {src['with_image']:>10,} {src['coverage_pct']:>9}%"
        )
    ...
    for t in stats["by_type"]:
        print(
            f"{t['type'][:30]:<30} {t['total']:>10,} {t['with_image']:>10,} {t['coverage_pct']:>9}%"
        )
    ...
    for c in stats["by_country"]:
        print(
            f"{c['country'][:30]:<30} {c['total']:>10,} {c['with_image']:>10,} {c['coverage_pct']:>9}%"
        )
    ...
    if stats["improvement_potential"]:
        ...
        for p in stats["improvement_potential"]:
            print(
                f"{p['source']:<30} {p['without_image']:>10,} {p['total']:>10,} {p['coverage_pct']:>9}%"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Batch update site images from multiple sources")
    parser.add_argument("--all", action="store_true", help="Run all image sources")
    parser.add_argument(
        "--source",
        "-s",
        type=str,
    ...
    parser.add_argument(
        "--limit", "-l", type=int, default=None, help="Maximum sites to process per source"
    )
    parser.add_argument("--stats", action="store_true", help="Print statistics only")
    parser.add_argument(
        "--test", type=str, metavar="SITE_NAME", help="Test image search for a site name"
    )
    parser.add_argument(
        "--sample", type=int, metavar="COUNT", help="Show sample sites without images"
    )
