...
## Source URL & Thumbnail research

When `needs_fix` includes source_url or thumbnail_url issues, research and suggest better values:

- **source_url**: Find the most authoritative, specific page about THIS site. Prefer in order:
  1. Dedicated Wikipedia article (any language) — a full article beats a fragment/subsection link
  2. UNESCO World Heritage page (for listed sites)
  3. Official archaeological project/museum page
  4. Authoritative database entry (Pleiades, ARIADNE, national heritage registry)
  - Replace fragment URLs (`#section` links) with dedicated articles whenever one exists
  - Non-Wikipedia sources are welcome when they're more specific or authoritative
  - Dead/broken URLs should be replaced with working alternatives

- **thumbnail_url**: Find a representative image of the site:
  1. Wikimedia Commons (via Wikidata P18 — already handled by Wave 1, but agents can fill gaps)
  2. Official site/museum image (must be publicly accessible, no auth walls)
  - Only suggest URLs that are direct image links (ending in .jpg/.png/.webp or from known CDNs)
  - Do NOT use Google Images thumbnails or hotlinked copyrighted photos
...
2. **Fragment-aware Wikipedia resolution** — for URLs with `#fragment` anchors (e.g. `Giza_pyramid_complex#Osiris_Shaft`), the script:
   a. Resolves the base article first (existing behavior)
   b. Extracts the fragment text (e.g. "Osiris Shaft") and tries it as a standalone Wikipedia article across priority languages (en, de, fr, it, es, ar)
   c. If a dedicated article exists with a different QID than the base article, prefers it (method: `url_wiki_fragment`, confidence 0.9)
   d. If neither fragment nor base resolve, tries the **site name** itself as a Wikipedia title across languages (method: `url_name_fallback`, confidence 0.85)
3. **W3C Reconciliation API** (`wikidata.reconci.link`) — batch POST site name + coordinates
4. **SPARQL geo-search fallback** — `SERVICE wikibase:around` with fuzzy name matching

~84 sites have meaningful Wikipedia fragment URLs. Without fragment-aware resolution, these resolve against the parent article and often get wrong QIDs (e.g. "Osiris Shaft" → Pyramid of Khentkaus I instead of Osiris-Grab).
