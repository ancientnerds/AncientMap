...
- `unclaimed` - HUMAN_ONLY D7: the text is the pre-March one, or the site is not in the snapshot.
  Checked alike; it claims no AI origin before or after.
...
**The trim** (`wc4.trim`): the piece occurs exactly once; it is not the whole sentence; it is not a
bare modifier (`wc4.bare_modifier`: nothing left once the V4 list's hedge, negation, contrast and
restriction words are out - `probably `, ` not`, `only `, `approximately `); the capital is restored
when the piece opened the sentence; and the rest has no problem of `sentence_problems` (length >= 25,
complete sentence, not garbled, no double space, no space before punctuation, no dangling
punctuation, balanced parentheses) that the sentence did not already have. A clause may go with its
own hedge (` (c. 3000 BC)`, `, dated to approximately 10,000 BC,`): 2,476 of the 15,133 sentences
(16.4 %) carry a hedged number, the March texts' commonest invention, and Phase 4's stricter rule
(no span with any such word) could only drop those sentences whole.
...
and `raw_data` is NULL when nothing else is left.
...
names of this run; numbering continues past every earlier WC plan (`--first-batch`, section 5).
    ...
    G="$PY $M/tools/write_gate4.py --group WC"; V="$PY $M/tools/verify_writes4.py --lane p4wc"

**0. Preconditions.** WA's last P4 step is accepted (the read must see WA's texts). The gates of
section 6 are green on the merged tree. `$M/logs/_write_apply_p4wc/` holds no batch of a plan you
do not name.
...
seed), never a re-judge of the same answers.
...
a pilot site the gate refused (`moved-since-check`) is asked again, as it should be.
...
named, in run order (the gate refuses an apply root holding batches of a plan not named):
...
| `build` | every site's decisions, pronoun rule, text, citations, record, raw_data, evidence; `wc_problems` on each | a due or unimported re-ask; `--first-batch` below 4001; a site never answered |
| `judge-import` | the judge round validates; quotes on the judge's own fetch; independence | `JUDGE_EXIT=1` below the pass mark |
| gate dry run | reads each site's live description and raw_data (read-only); plans | `written-by-p4` (a P4 text since), `moved-since-check` (the pair is neither the checked one nor the outcome's) - per site, the rest of the batch goes on |
| gate plan (`validate_rows`) | site-atomic pairs (kept, clear, or raw_data alone for a byte-identical text), the evidence's transition, `wc_problems`, no key outside the three changes, no full provenance | the whole batch |
...
| `verify_writes4 --lane p4wc` | the chain, `wc_problems` and T08 on every written site, and that the live description, citations and check record are exactly what the journal evidence of its last WC write composes | any deviation: the step is not accepted |
...
- `mass4` digests `scripts/remediation/phase4/*.py`: this lane adds `wc4.py` and changes
  `write4.py`, so it is merged into a tree only when no mass4 run executes from that tree.
