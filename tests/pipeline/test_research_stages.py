...
class TestClusterResult:
    def test_default_error_empty(self):
        r = ClusterResult(prose="test", sources=[], score=0, passed=False)
        assert r.error == ""

    def test_has_expected_fields(self):
        r = ClusterResult(prose="body", sources=[{"citation": 1}], score=85, passed=True)
        assert r.prose == "body"
        assert r.score == 85
    ...
    def test_error_field_set(self):
        r = ClusterResult(
            prose="", sources=[], score=0, passed=False, error="No sources found."
        )
    ...
    def test_sources_list(self):
        ...
        r = ClusterResult(prose="text", sources=sources, score=72, passed=True)
