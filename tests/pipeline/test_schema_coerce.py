...
# --- MiniMax M3 nested-$text recovery -------------------------------------
# Real Theo run 2026-06-16: M3 encoded an array of `caveats` strings as a
# single nested {"$text": ..., "item": {...}} chain (with stray "</item>"
# markers) at caveats[0]. The old coercer saw "dict where string expected"
# and DROPPED all six caveats. We must recover the strings instead.


def test_recovers_nested_text_chain_in_array_of_strings() -> None:
    schema = {"type": "array", "items": {"type": "string"}}
    raw = [
        {
            "$text": "first caveat</item>",
            "item": {
                "$text": "second caveat</item>",
                "item": "third caveat</item>",
            },
        }
    ]
    assert _coerce_to_schema(raw, schema) == [
...
def test_recovers_bare_nested_text_chain_when_array_expected() -> None:
    # M3 sometimes drops the wrapping array entirely and returns the chain
    # as the whole field value.
    schema = {"type": "array", "items": {"type": "string"}}
    raw = {"$text": "only caveat</item>"}
    assert _coerce_to_schema(raw, schema) == ["only caveat"]


def test_recovers_text_node_for_string_field() -> None:
    schema = {"type": "string"}
    assert _coerce_to_schema({"$text": "a value</item>"}, schema) == "a value"


def test_text_recovery_mixes_with_plain_strings() -> None:
    schema = {"type": "array", "items": {"type": "string"}}
    raw = ["plain one", {"$text": "wrapped</item>", "item": "wrapped two</item>"}]
    assert _coerce_to_schema(raw, schema) == ["plain one", "wrapped", "wrapped two"]
